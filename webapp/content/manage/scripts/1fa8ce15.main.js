!function() {
    ...
    define("angular", [ "jquery" ], function(global) {
            ...
            Service.prototype = _.extend(Service.prototype, {
                    ...
                    return this.getList().then(function(clusters) {
                        if (clusters.length) {
                            ...
                            if ("undefined" != typeof Storage) {
                                var lastClusterId = JSON.parse(localStorage.getItem("cluster")), lastCluster = void 0;
                                _.isUndefined(lastClusterId) || (lastCluster = _.find(clusters, function(cluster) {
                                    return cluster.id === lastClusterId;
                                })), _.isUndefined(lastCluster) ? localStorage.setItem("cluster", JSON.stringify(cluster.id)) : cluster = lastCluster;
                            }
                ...
                getList: function() {
                    return this.restangular.all("cluster").getList().then(function(clusters) {
                        return clusters = _.sortBy(clusters, function(cluster) {
                            return cluster.name;
                        });
                    });
                },
                ...
                switchCluster: function(cluster) {
                    this.clusterModel = cluster, this.clusterId = cluster.id;
                },
                base: function() {
        ...
        var PoolService = function(ClusterService) {
            ...
            return Service.prototype = _.extend(Service.prototype, {
                ...
                getFull: function(id) {
                    return id = _.isString(id) ? parseInt(id, 10) : id, this.restangular.clusterFull().one("pool", id).get().then(function(pool) {
                        return pool;
                    });
                },
                remove: function(id) {
        ...
        var OSDService = function(ClusterService) {
            ...
            return Service.prototype = _.extend(Service.prototype, {
                ...
                getFull: function(id) {
                    return id = _.isString(id) ? parseInt(id, 10) : id, this.restangular.clusterFull().one("osd", id).get().then(function(osd) {
                        return osd;
                    });
                },
                patch: function(id, update) {
    ...
    angular.module("angular-growl", []), angular.module("angular-growl").directive("growl", [ "$rootScope", function($rootScope) {
        ...
        define("controllers/root", [ "lodash", "helpers/server-helpers", "helpers/cluster-settings-helpers", "helpers/cluster-response-helpers", "helpers/modal-helpers" ], function(_, ServerHelpers, ClusterSettingsHelpers, ResponseHelpers, ModalHelpers) {
            var RootController = function($q, $log, $timeout, $rootScope, $location, $scope, KeyService, ClusterService, ToolService, ServerService, $modal, OSDConfigService, RequestTrackingService, config) {
                ...
                var promises = [ KeyService.getList(), ToolService.config(), OSDConfigService.getFull() ], start = Date.now();
        ...
        define("controllers/pool", [ "lodash", "helpers/modal-helpers", "helpers/error-helpers" ], function(_, ModalHelpers, ErrorHelpers) {
            var PoolController = function($q, $log, $scope, PoolService, ClusterService, $location, $modal, RequestTrackingService, $rootScope, $timeout, config) {
                ...
                function refreshPools() {
                    ...
                    $rootScope.keyTimer = void 0), PoolService.getListFull().then(function(result) {
                        $scope.pools = updatePools(copyPools(result.data), $scope.pools);
                        var headers = result.headers("Allow");
                        $scope.isAddAllowed = headers && headers.indexOf("POST") > 0, $scope.isDeleteAllowed = $scope.isAddAllowed;
                ...
                $scope.isAddAllowed = !1, $scope.isDeleteAllowed = !1;
        ...
        define("controllers/osd-host", [ "lodash", "helpers/modal-helpers", "helpers/osd-helpers", "helpers/error-helpers" ], function(_, ModalHelpers, OsdHelpers, ErrorHelpers) {
            var disableRepairCommand = !0, text = {
                ...
                ServerService.get($scope.fqdn).then(function(server) {
                    ...
                    }), r.ids.length > 0 && OSDService.getFull(r.ids[0]).then(function(osd) {
        ...
        define("controllers/pool-modify", [ "lodash", "helpers/pool-helpers", "helpers/modal-helpers", "helpers/error-helpers" ], function(_, PoolHelpers, ModalHelpers, ErrorHelpers) {
            var PoolModifyController = function($log, $q, $scope, PoolService, ClusterService, CrushService, ToolService, $location, $routeParams, $modal, RequestTrackingService) {
                ...
                $scope.modify = !1, $scope.clusterName = ClusterService.clusterModel.name, $scope.breadcrumbs = [ {
                    ...
                    text: "编辑",
                    active: !0
                } ], $scope.id = $routeParams.id, $scope.isUpdateAllowed = !1, $scope.isDeleteAllowed = !1, 
                $scope.isEdit = !0, $scope.cancel = function() {
                    $location.path("/pool");
                ...
                $q.all(promises).then(function(results) {
                    ...
                    $scope.isUpdateAllowed = headers && headers.indexOf("PATCH") > 0, $scope.isDeleteAllowed = headers && headers.indexOf("DELETE") > 0, 
                    $scope.defaults = angular.copy($scope.pool), this.crushrulesets = results[1], $scope.crushrulesets = PoolHelpers.normalizeCrushRulesets(this.crushrulesets), 
                    $scope.up = !0, PoolHelpers.addWatches($scope);
                }.bind(this));
            };
    ...
    function() {
        ...
        "use strict";
        define("controllers/cluster", [ "lodash" ], function(_) {
            var ClusterController = function($location, $route, $scope, ClusterResolver, ClusterService, config, $modal, $http) {
                ClusterResolver.then(function() {
                    return null === ClusterService.clusterId ? void $location.path(config.getFirstViewPath()) : ($scope.clusterDropdownTemplate = "views/cluster-dropdown.html", 
                    $scope.selectedCluster = ClusterService.clusterModel, $scope.selectedCluster.id_short = ClusterService.clusterModel.id.substring(0, 8), 
                    void ClusterService.getList().then(function(clusters) {
                        _.each(clusters, function(cluster) {
                            cluster.id_short = cluster.id.substring(0, 8);
                        }), $scope.clusters = clusters;
                    }));
                }), $scope.switchCluster = function(cluster) {
                    $scope.selectedCluster && cluster.id === $scope.selectedCluster.id || ("undefined" != typeof Storage && localStorage.setItem("cluster", JSON.stringify(cluster.id)), 
                    $scope.selectedCluster = cluster, ClusterService.switchCluster(cluster), $route.reload());
                };
            };
        ...
        define("git", [ "angular" ], function() {
            var runBlock = function() {
                window.inktank = {
                    commit: "v1.3.1.1-0-3d9c917-dirty"
                };
            };
            return [ runBlock ];
        });
